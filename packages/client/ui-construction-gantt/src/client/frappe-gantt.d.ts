 ...
 *
 * The upstream package ships no types of its own and the community
 * `@types/frappe-gantt` targets the 0.x API, so this package pins the 1.2.2
 * surface it actually calls: the default `Gantt` class constructor, the
 * `change_view_mode` / `refresh` methods, and the option and task shapes used
 * by the read-only adapter.
 */
