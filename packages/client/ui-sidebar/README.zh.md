...
呈现模式：默认的 `business` 模式下，外壳不渲染通往技术界面的面板行——目前是 `plugins` 标记，`terminal` 预留给该类面板注册后生效。`expert` 模式显示所有已注册行。标记是稳定的面板 id（即寻址 `main` keyed slot 的同一个 id），因此包按 id 让入口加入过滤，外壳从不 import 注册方。该模式只影响呈现，绝不过滤审批、设置席位或模式开关本身；它经主题快照钩子（`useUiMode`）到达。
