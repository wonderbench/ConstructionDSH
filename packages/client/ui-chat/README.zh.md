...
输出降噪 Beta 开关开启（`body[data-dsw-output-denoise]`，默认关闭）且处于业务界面模式时，已关闭轮次中已结算的 Assistant 正文若散文超过三个非空源文本行则折叠：逐字取自模型原文的第一行作为一句结论，其余内容收进「查看说明」展开控件，展开后渲染未改动的 `AssistantMarkdown` 与原始块——逐字保证由构造成立，一字不改、一字不丢。散文折叠期间，思考行、图片与工具调用头保持可见。轮次未关闭、正文运行中或已中断、专家模式以及开关关闭时，呈现与改动前完全一致；展开状态是会话内的本地状态，不跨刷新持久化。结论行是模型文本的逐字切片，绝不推导成功或失败——完成判定仍依据工具结果与交付事件，模型「圆满完成」式的声明只作为正文呈现。模式在渲染时读取 body 属性，因此切换设置后由下一次席位驱动的重渲染生效，而不是通过订阅。
