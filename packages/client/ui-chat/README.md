...
With the output-denoise Beta flag on (`body[data-dsw-output-denoise]`, default off) and business UI mode, the settled Assistant body of a closed Turn folds when its prose exceeds three non-empty source lines: the verbatim first line stands as the one-line conclusion and the rest opens through a `View explanation` disclosure whose expansion renders the untouched `AssistantMarkdown` over the original blocks — verbatim by construction, one character unchanged and one not lost. Reasoning rows, images, and Tool-call heads stay visible while prose folds. An open Turn, a running or interrupted body, expert mode, and the flag off all render the pre-change shape exactly, and the expansion state is per-render local state — in-session only, never persisted. The conclusion line is a verbatim slice of the model text and never derives success or failure; completion judgement stays with Tool results and deliverable events, so a model claim such as "圆满完成" presents as prose only. The mode is read from the body attributes at render time, so a settings toggle applies on the next seat-driven re-render rather than through a subscription.
