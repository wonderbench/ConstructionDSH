...
describe('deliverables layout', () => {
  it('keeps the first file-section offset and removes the second', () => {
    ...
    expect(deliveries).toMatch(new RegExp(
      '\\.description\\s*\\{[^}]*font-size:\\s*var\\(--dsw-ui-font-secondary\\);'
        + '[^}]*line-height:\\s*var\\(--dsw-ui-line-secondary\\)', 's',
    ))
    expect(deliveries).toMatch(new RegExp(
      '\\.open\\s*\\{[^}]*padding:\\s*4px 8px;[^}]*font-size:\\s*var\\(--dsw-ui-font-secondary\\);'
        + '[^}]*line-height:\\s*var\\(--dsw-ui-line-secondary\\)', 's',
    ))
