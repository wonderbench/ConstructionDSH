...
describe('ui-theme host', () => {
  it('registers, validates, and disposes the durable theme namespace with its fiber', async () => {
    ...
    expect(ctx.settings.get(ns)).toEqual({ preference: DEFAULT_PREFERENCE, fontSize: 14, outputDenoise: false, uiMode: 'business' })
    await ctx.settings.update(ns, { preference: 'dark', fontSize: 16 })
    expect(ctx.settings.get(ns)).toEqual({ preference: 'dark', fontSize: 16, outputDenoise: false, uiMode: 'business' })
    await ctx.settings.update(ns, { preference: 'dark', fontSize: 16, outputDenoise: true })
    expect(ctx.settings.get(ns)).toEqual({ preference: 'dark', fontSize: 16, outputDenoise: true, uiMode: 'business' })
    await ctx.settings.update(ns, { uiMode: 'expert' })
    expect(ctx.settings.get(ns)).toEqual({ preference: 'dark', fontSize: 16, outputDenoise: true, uiMode: 'expert' })
    await expect(ctx.settings.update(ns, { preference: 'sepia' })).rejects.toThrow()
    await expect(ctx.settings.update(ns, { uiMode: 'kiosk' })).rejects.toThrow()
  ...
  it('answers each collection with the current durable preference until disposal', async () => {
    ...
    await ctx.settings.update(THEME_SETTINGS_NAMESPACE, { preference: 'dark', fontSize: 17, uiMode: 'expert' })
    ...
    expect(rowText(collect(ctx)[1])).toContain('setAttribute("data-dsw-ui-mode", "expert")')
    await fiber.dispose()
