...
export const guideEn = {
  ...
  guideDrawingSplitIntro: 'Choose Drawing split mode for a new task that starts from a stack of drawing PDFs. Name the files or folder, the split granularity, and where the outputs should land.',
  guideDrawingSplitExplanation: [
    '### How splitting works',
    'The agent inspects each PDF, then splits by page range, per page, or top-level bookmark with construction_pdf_split. It delivers the output files and an index, then stops — no structured decomposition and no follow-up business work.',
    '### When to choose it',
    'Use it when the next step needs separate drawing files: handing subsets to specialists, reading parts with the file tools, or archiving a drawing set by discipline.',
  ].join('\n\n'),
  guideDrawingSplitUsage: [
    '### Split a multi-drawing PDF',
    '> Split 设计院图纸-地下室结构.pdf by top-level bookmark into out/drawings/, one PDF per specialty, and write an index of the parts.',
    'Expected output: one output PDF per bookmark section plus an index file listing each part and its page range.',
    '### Split by page range',
    '> Take pages 1-40 of 暖通平面图.pdf as one pack and pages 41-80 as another; keep the originals untouched.',
    'Expected output: two output PDFs with the requested ranges and a short manifest of what each contains.',
  ].join('\n\n'),
  guideEngineeringIntro: 'Choose Engineering mode when the task mixes coding with construction deliverables: reading project documents, splitting drawings, costing, scheduling, or reporting.',
  guideEngineeringExplanation: [
    '### What it adds',
    'Engineering mode is Standard mode plus the construction runtime: engineering file reading tools, drawing-PDF splitting, costing, scheduling, report export, and the cost, quality, safety, and schedule business Skills with their task-binding rules.',
    '### When to choose it',
    'Pick it for takeoffs, schedule analysis, document review, or any task that should produce construction artifacts instead of code alone. Plain coding tasks stay in Standard mode.',
  ].join('\n\n'),
  guideEngineeringUsage: [
    '### Price a bill of quantities',
    '> 用清单与定额口径复核 01-造价/地下室清单.xlsx 的综合单价，标出异常项并导出对比表。',
    'Expected output: a comparison export under the workspace artifacts directory, with blank or missing prices flagged rather than treated as zero.',
    '### Plan the structure works',
    '> 按 进度计划任务表.xlsx 和项目日历计算地下室结构进度，给出关键线路与总工期。',
    'Expected output: a schedule calculation with the critical path, plus a presentable chart when one is requested.',
  ].join('\n\n'),
...
export const guideZh: Record<PresetGuideKey, string> = {
  ...
  guideDrawingSplitIntro: '新建任务时从一摞图纸 PDF 开始时选择「图纸拆解模式」。说明文件或目录、拆分粒度，以及输出放的位置。',
  guideDrawingSplitExplanation: [
    '### 拆分方式',
    'Agent 逐份检查 PDF，再用 construction_pdf_split 按页范围、逐页或一级书签拆分，交付输出文件与索引后即停止——不做结构化拆解，也不自动跟进后续业务。',
    '### 什么时候选',
    '当后续步骤需要独立图纸文件时使用：按专业分发给相关人员、用文件工具分册阅读，或按专业归档一套图纸。',
  ].join('\n\n'),
  guideDrawingSplitUsage: [
    '### 拆分多专业图纸 PDF',
    '> 把 设计院图纸-地下室结构.pdf 按一级书签拆到 out/drawings/，每个专业一个 PDF，并生成分册索引。',
    '预期产出：每个书签章节一个输出 PDF，以及列出各分册页码范围的索引文件。',
    '### 按页范围拆分',
    '> 取 暖通平面图.pdf 第 1-40 页作为一个包、第 41-80 页作为另一个包，保留原文件不动。',
    '预期产出：两个按指定页码范围拆出的 PDF，以及一份说明各自内容的简短清单。',
  ].join('\n\n'),
  guideEngineeringIntro: '当任务把编程与工程交付混在一起时选择「工程专业模式」：读取项目文档、拆分图纸、造价、进度或报告。',
  guideEngineeringExplanation: [
    '### 增加了什么',
    '工程专业模式 = 标准模式 + 工程运行时：工程文件读取工具、图纸 PDF 拆分、造价、进度、报告导出，以及造价、质量、安全、进度四个行业 Skills 及其任务绑定规则。',
    '### 什么时候选',
    '算量、进度分析、文档审查，或任何应当产出工程成果而非仅代码的任务都选它。纯编程任务仍用标准模式。',
  ].join('\n\n'),
  guideEngineeringUsage: [
    '### 复核清单综合单价',
    '> 用清单与定额口径复核 01-造价/地下室清单.xlsx 的综合单价，标出异常项并导出对比表。',
    '预期产出：工作区成果目录下的对比导出，空单价或缺单价会被标出而不是按零处理。',
    '### 计算结构施工进度',
    '> 按 进度计划任务表.xlsx 和项目日历计算地下室结构进度，给出关键线路与总工期。',
    '预期产出：带关键线路的进度计算结果；需要时附可展示的图表。',
  ].join('\n\n'),
