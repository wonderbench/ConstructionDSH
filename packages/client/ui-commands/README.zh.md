...
与 `ui-input-trigger` 及 `ui-conversation` 一起挂载本插件；`/` source 随即出现在触发菜单中，业务包经 `ctx.commandUi` 注册自己的命令表面。键入 `/model` 打开已注册的弹窗；带参数声明的宿主命令打开其输入或直接执行。弹窗持有 composer 焦点：键入即在已加载的行上本地筛选，`↑`／`↓` 在行间移动，回车与 `Tab` 接受高亮行，Escape 与 `Shift+Tab` 把焦点还给 composer。高亮落在选项标记为会话当前值的行上，因此在刚打开的弹窗上接受即确认当前值。composer 的 `+` 按钮与键入的 `/` 打开同一个菜单：「添加」小节（文件、目标、计划、反馈）、「功能」小节与「指令」小节（压缩、权限、模型、下载日志）按展示顺序排列，每行带图标、本地化的标题与说明，本地化标题与命令名不同时还显示命令名作为别名。「添加」与「指令」列表是此客户端的既有使用频次排位；宿主描述声明 `section`（`'add' | 'functions' | 'commands'`）时，其行按目录顺序归入对应分组标题之下——工程业务 Skill 正是借此进入「功能」小节。
...
内置命令定义携带稳定的 `definitionId`。客户端按标识选择本地化标题、说明、图标和输入写法，修改宿主说明不会改变选择结果。展示面归属包含四个建筑业务 Skill 行（`construction-safety`、`construction-quality`、`construction-cost`、`construction-schedule`），其展示面只声明标题与图标，因此这些行渲染为「标题 + 命令名」，右侧无说明文字。没有匹配标识的同名覆盖保留自己的文案，也不获得内置别名。在任何界面语言下，中英文写法都通过同一个会话有效目录解析，草稿保留手输写法，提交使用宿主注册名。贡献项提供自己的 `label`、`description` 和 `icon`，每次生成候选项时读取。空查询的小节顺序遵循既有列表，未声明的未列出行按目录顺序排在「指令」末尾。
