...
describe('web e2e: preset roster guidance', () => {
  ...
  it('reads mode details and examples without changing the new-task default', async () => {
    ...
    const trigger = settings.getByRole('button', { name: '模式说明: 工程专业模式', exact: true })
    await trigger.click()
    const guide = page.getByRole('dialog', { name: '工程专业模式', exact: true })
    await guide.getByRole('heading', { name: '增加了什么', exact: true }).waitFor()
    await guide.getByRole('tab', { name: '如何使用', exact: true }).click()
    await guide.getByRole('heading', { name: '复核清单综合单价', exact: true }).waitFor()
