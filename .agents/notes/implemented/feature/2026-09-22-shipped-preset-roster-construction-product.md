...
Homepage picker order is `standard` (order 1), `drawing-split` (order 2), `engineering` (order 3). The four declarations are the web-app bundle's `presets/<id>.patch.yml` files, listed in `dsh.bundle.patch`. `cordis` stays fully usable — settings roster cards, the read-only composition viewer, and the creator-draft flow all still reach it — but its declaration carries `config.picker: false`, an optional boolean that hides the preset from the new-session menu only. `AgentPresetSeat` filters `picker: false` rows out of its menu at render time and nowhere else: a session already running one still gets its label, and the settings section never filters. The field defaults to visible when absent, and a `picker` value that is present but not a boolean fails the preset row's config validation instead of silently picking a side. The creator-draft entry stages `cordis` directly through the seat controller, so it never depended on the filtered menu roster.
...
Web and CLI lanes that used the shipped `ptc` or `minimal` preset as a composition to mount now seed lane-owned `PresetDefinition` constants from `apps/web/tests/fixtures/presets/definitions.ts`, keeping the recorded-session fixtures' `agentPreset` headers valid without resurrecting the presets as shipped surface. The `ptc-round` and `minimal-preset` recorded Web scenarios are deleted with their snapshot directories; the PTC escalation and presentation scenarios survive on the lane-owned preset.
...
`drawing-split` mounts the drawing-only runtime, so its persona can execute the workflow it names; the business surface — Skills, commands, task tools — stays exclusive to `engineering`, and the shipped-roster spec pins both compositions (drawing-split's row carries `business: false`; engineering's row carries no config) so the split cannot drift quietly. V2 sessions recorded under the legacy `code` preset id still migrate to `ptc` in the session-format layer, but no shipped preset answers to `ptc` anymore: a deployment without its own `ptc` declaration fails at mount with the roster's not-found reason, the session-format catalog's migration spec keeps pinning the `code` → `ptc` mapping, and the Web migration snapshot records selections over the current roster instead.

## Verification

The `agent-preset-registry` suite pins registration, the roster's `picker` mapping, activation diagnostics, and the preset-scoped command listing (a command registered inside one preset's standing composition lists only for agents joined to it); the `ui-agent-preset` client suites pin the locale keys, the menu-only filter, and the chip label for a running picker-hidden preset. The `web-agent-presets` CLI e2e boots the shipped bundle patches with the lane-owned PTC and minimal rows and pins both compositions. The Web lanes `agent-preset-selection`, `agent-preset-authoring`, and `settings-chrome` regenerate their goldens through the repository's refresh mode, and the recorded-session snapshot cases that referenced the removed presets were deleted with their fixtures.
