...
读者可以检查和复制已记录的程序，无需修改回放数据。Schema 没有可识别的语言提示时不提供语法高亮。组件测试覆盖记录工具名识别、Schema 回退、源码与参数原样复制、输出状态及独立换行。JSON 树测试覆盖裁剪几何、缺少 `ResizeObserver`、悬停切换或卸载后剪贴板写入落定，以及悬停期间读取值的次数。思考测试覆盖手动展开折叠及 Trajectory 记录切换。固定组装后检查器展示并验证溢出与原始 JSON 往返切换的浏览器场景，随随附的 `ptc` preset 一并移除；该移除由[随附 preset roster 记录](2026-09-22-shipped-preset-roster-construction-product.zh.md)承担。
