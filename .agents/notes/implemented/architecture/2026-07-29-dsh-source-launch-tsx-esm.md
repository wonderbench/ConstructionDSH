...
A source launch also registers `apps/cli/source-plane.mjs`, which redirects every `packages/<group>/<pkg>/lib` resolution to its existing `src` counterpart: the profile loader reaches plugin entrypoints through package `exports` (`lib`) while tsx projects workspace imports to `src`, and two module planes split `unique symbol` identities such as `TOOL_RUNTIME_SCHEDULER`. The built bin keeps the artifact plane; installs without package sources leave the redirect inert.
